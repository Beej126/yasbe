...
@IncrementalID int,
@OverrideExistingFinalizedMediaSubsets BIT = 0,
...
IF (@OverrideExistingFinalizedMediaSubsets = 0)
BEGIN
  DECLARE @MediaSubsetCount INT, @CR VARCHAR(2)
  SET @CR = CHAR(13)+CHAR(10)
  SELECT @MediaSubsetCount = MAX(MediaSubsetNumber) FROM MediaSubset WHERE IncrementalID = @IncrementalID AND Finalized = 1
  IF (@MediaSubsetCount > 0)
  BEGIN
    RAISERROR('[CONFIRM] There are %d existing *FINALIZED* Discs defined in this Incremental Backup.%sSelect [Override] if you''re sure you want to lose them.', 16,1, @MediaSubsetCount, @CR)
    RETURN
  END
END

-- clear out any existing non finalized MediaSubsets above the default "0" from previous runs of this proc
DELETE MediaSubset WHERE IncrementalID = @IncrementalID AND MediaSubsetNumber > 0
-- clear out any file archive records from previous runs of this proc
DELETE fa FROM FileArchive fa JOIN MediaSubset ms ON ms.MediaSubsetID = fa.MediaSubsetID AND ms.IncrementalID = @IncrementalID
...
-- 2) insert the set of files which have been successfully backed up previously (ie MediaSubset.Finalized=1) but have *changed* since then
INSERT FileArchive(MediaSubsetID, FileID, ModifiedDate, [Size])
...
WHERE NOT EXISTS(
  SELECT 1
  FROM FileArchive fa
  JOIN MediaSubset ms ON ms.MediaSubsetID = fa.MediaSubsetID
  WHERE fa.FileID = f2.FileID AND fa.ModifiedDate = f1.ModifiedDate AND fa.[Size] = f1.[Size]
  AND ms.Finalized = 1)
